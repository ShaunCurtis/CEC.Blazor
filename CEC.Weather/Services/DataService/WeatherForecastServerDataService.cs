...
{
    public class WeatherForecastServerDataService : BaseDataService<DbWeatherForecast>, IWeatherForecastDataService
    {
        ...
        private List<SqlParameter> GetSQLParameters(DbWeatherForecast item, bool isinsert = false)
        {
            ...
            if (isinsert) parameters.Insert(0, new SqlParameter("@WeatherForecastID", SqlDbType.Int) { Direction = ParameterDirection.Input, Value = item.ID });
            else parameters.Insert(0, new SqlParameter("@WeatherForecastID", SqlDbType.Int) { Direction = ParameterDirection.Output });
            return parameters;
        }
