...
namespace CEC.Blazor.Server.Data
{
    public class WeatherForecastDbContext : DbContext
    {
        ...
        /// <returns></returns>
        internal async Task<DbTaskResult> RunDatabaseStoredProcedureAsync(string storedprocname, List<SqlParameter> parameters, RecordConfigurationData recordConfiguration)
        {
            var rows = await this.Database.ExecuteSqlRawAsync(this.GetParameterizedSqlCommand(storedprocname, parameters));
            if (rows == -1)
            {
                var outputparam = parameters.First(item => item.Direction == ParameterDirection.Output && item.SqlDbType == SqlDbType.Int);
                ...
                if (outputparam != null) ret.NewID = Convert.ToInt32(outputparam.Value);
                return ret;
            }
